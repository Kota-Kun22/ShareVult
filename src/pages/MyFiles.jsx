...
//Fetches the user's files from the backend
  const fetchFiles= async()=>{
    ...
    try {
      ...
      console.log("Error fetching files from the server:",error);
  ...
  const togglePublic = async (fileToUpdate)=>{
    try {
      ...
      toast.success(`File is now ${fileToUpdate.isPublic ? 'private' : 'public'}.`);

    } catch (error) {
      console.log("Error toggling file public status:",error);
      toast.error('Failed to update file sharing status. Please try again later.',error.message);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error('Failed to delete file. Please try again later.',error.message);
                      ...
                      <div className="grid grid-cols-3 gap-4">
                         <div 
                          ...
                          className="flex justify-center">
                            <button 
                            title="Download"
                            className="text-gray-500:text-green-600">
                            <Download size={18}/>
                            </button>
                         </div>
